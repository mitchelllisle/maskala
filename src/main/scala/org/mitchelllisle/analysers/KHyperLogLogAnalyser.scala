...
object KHyperLogLogAnalyser {
  ...
  /** Estimates the number of unique values in the dataset based on the KHLL algorithm.
    ...
    * @param k
    *   The number of minimum hashes to consider, default is 2048.
    * @return
    *   DataFrame with a single column "estimatedNumValues" containing the estimated number of unique values.
    */
  private def estimateNumValues(data: DataFrame, numHashes: Long, k: Int = 2048): DataFrame = {
    val distinctLargerThanKLogic = (
      F.lit(k - 1) *
        (
          F.pow(F.lit(2), F.lit(64)) / (
            F.max("h")
              + F.lit(1)
  ...
  /** Prepares the source table by transforming specified columns into JSON format.
    ...
    * @param columns
    *   The columns to be transformed and concatenated into a JSON string.
    * @param primaryKey
    *   The primary key column name.
    * @return
    *   Transformed DataFrame with "value" and "id" columns in JSON format.
    */
  ...
  /** Creates the KHLL table by calculating the k lowest hashes and aggregating using HyperLogLog.
    ...
    *   The number of minimum hashes to consider, default is 2048.
    * @return
    *   KHLL DataFrame with hash values and HyperLogLog estimates.
    */
  ...
  /** Calculates the value sampling ratio, which is the estimated ratio of values captured in the sketch.
    *
    * @param data
    *   The DataFrame containing the estimated number of unique values.
    * @param k
    *   The number of minimum hashes, default is 2048.
    * @return
    *   DataFrame with a single column "valueSamplingRatio" representing the sampling ratio.
    */
  ...
  /** Calculates the estimated uniqueness distribution across different levels of uniqueness.
    ...
    * @param khllTable
    *   The KHLL DataFrame.
    * @param valueSamplingRatioTable
    *   The DataFrame containing the value sampling ratio.
    * @return
    *   DataFrame with the distribution of estimated value counts and ratios for each level of uniqueness.
    */
  private def calculateEstimatedUniquenessDistribution(
      khllTable: DataFrame,
      valueSamplingRatioTable: DataFrame,
      numHashes: Long
  ): DataFrame = {
  ...
  /** Calculates the cumulative distribution of uniqueness, adding up the counts and ratios up to each level of
    * uniqueness.
    *
    * @param estimatedUniquenessDistribution
    *   DataFrame containing the estimated uniqueness distribution.
    * @return
    *   DataFrame with cumulative counts and ratios of uniqueness.
    */
  ...
  /** Orchestrates the KHLL analysis by applying all steps in sequence and returning the final DataFrame.
    *
    * @param dataFrame
    *   The original DataFrame to be analyzed.
    * @param groupByColumns
    *   Columns used for grouping in the source table creation.
    * @param userIdColumn
    *   User ID column name for source table creation.
    * @param k
    *   The number of minimum hashes, default is 2048.
    * @return
    *   The final DataFrame representing the cumulative distribution of uniqueness.
    */
  def apply(dataFrame: DataFrame, groupByColumns: Seq[String], userIdColumn: String, k: Int = 2048): DataFrame = {
    ...
    val estimatedUniquenessDistribution =
      calculateEstimatedUniquenessDistribution(khllTable, valueSamplingRatio, numHashes)
